---
name: delete-integration-opening
description: 清理数据库中售中开通工单及关联明细，保留指标结果和本地文件。用户明确要求清理 integration_opening 时使用；不在取数、算数或月报流程中自动调用。
---

# 一体化：售中开通工单数据清理

只负责售中开通工单清理，不取数、不计算指标、不绘制 PPT。对应取数技能为 `collect-integration-opening`，只调用已有清理入口，不另写 SQL 删除。

## 数据口径

- 数据集：`integration_opening`
- 源记录主键：工单号；按已有记录与批次关系定位，不按主键重新计算业务口径。
- 日期口径：采集按工单结束时间筛选，删除按 ETL 周期选批次。
- 关联清理范围：raw_source_record、raw_source_record_version 及对应算数明细，仅删除选中批次和脚本已验证的关联记录，不清空共享表。
- 入口：`delete/integration_opening.py`，使用模块方式 `python -m delete.integration_opening` 运行。
...
- 与对应 collect 一致：默认取对话中的月报月份第一天至最后一天。
...
```powershell
py -3.11 -m delete.integration_opening --database data/quality_assessment.db --start-date 2026-08-01 --end-date 2026-08-31
```
