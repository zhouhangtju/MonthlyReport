---
name: delete-eoms-terminal-removal
description: 清理数据库中终端回收服务类工单及关联明细，保留指标结果和本地文件。用户明确要求清理 eoms_service_removal_order 时使用；不在取数、算数或月报流程中自动调用。
---

# EOMS：终端回收服务类工单数据清理

只负责终端回收服务类工单清理，不取数、不计算指标、不绘制 PPT。对应取数技能为 `collect-eoms-terminal-removal`，只调用已有清理入口，不另写 SQL 删除。

## 数据口径

- 数据集：`eoms_service_removal_order`
- 源记录主键：工单号；按已有记录与批次关系定位，不按主键重新计算业务口径。
- 日期口径：服务类工单实际成功入库周期。
- 关联清理范围：原始记录、版本、终端源快照及快照行数据、对应算数明细，仅删除选中批次和脚本已验证的关联记录，不清空共享表。
- 入口：`delete/eoms_terminal_removal.py`，使用模块方式 `python -m delete.eoms_terminal_removal` 运行。
...
- 对应 collect 文档默认向前推 1 个日历月的同一日期，目标月没有同一日时取月末。注意该文档跨月示例与采集脚本同月限制存在出入：不得把文档示例当作已入库事实；必须核对实际成功 ETL 的日期，不一致时停止确认，不修改 collector。
...
```powershell
py -3.11 -m delete.eoms_terminal_removal --database D:/MonthlyReport/data/quality_assessment.db --start-date 2026-07-31 --end-date 2026-08-31
```
