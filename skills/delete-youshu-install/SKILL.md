---
name: delete-youshu-install
description: 清理数据库中企宽新装工单及关联明细，保留指标结果和本地文件。用户明确要求清理 youshu_install 时使用；不在取数、算数或月报流程中自动调用。
---

# 有数：企宽新装清单数据清理

只负责企宽新装工单清理，不取数、不计算指标、不绘制 PPT。对应取数技能为 `collect-youshu-install`，只调用已有清理入口，不另写 SQL 删除。

## 数据口径

- 数据集：`youshu_install`
- 源记录主键：工单id；按已有记录与批次关系定位，不按主键重新计算业务口径。
- 日期口径：采集按派单时间筛选，按 ETL 日期及地市批次定位。
- 关联清理范围：raw_source_record、raw_source_record_version 及对应算数明细，仅删除选中批次和脚本已验证的关联记录，不清空共享表。
- 入口：`delete/youshu_install.py`，使用模块方式 `python -m delete.youshu_install` 运行。
...
- 与对应 collect 一致：未显式指定日期时，结束日期取对话中的月报日期，开始日期向前推 3 个日历月的同一日期。采集使用 --chunk-days 100 并按 11 地市导出；清理核对全部日期与地市批次，不能只选一个地市代表整个周期。
...
```powershell
py -3.11 -m delete.youshu_install --database data/quality_assessment.db --start-date 2026-05-31 --end-date 2026-08-31
```
