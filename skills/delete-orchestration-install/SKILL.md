---
name: delete-orchestration-install
description: 清理数据库中互联网专线新装单及关联明细，保留指标结果和本地文件。用户明确要求清理 orch_install 时使用；不在取数、算数或月报流程中自动调用。
---

# 编排：互联网专线新装单数据清理

只负责互联网专线新装单清理，不取数、不计算指标、不绘制 PPT。对应取数技能为 `collect-orchestration-install`，只调用已有清理入口，不另写 SQL 删除。

## 数据口径

- 数据集：`orch_install`
- 源记录主键：订单号；按已有记录与批次关系定位，不按主键重新计算业务口径。
- 日期口径：采集按派单时间筛选，删除按 ETL 分段周期选批次。
- 关联清理范围：ods_orch_install、原始记录、版本及对应算数明细；不处理编排开通，仅删除选中批次和脚本已验证的关联记录，不清空共享表。
- 入口：`delete/orchestration_install.py`，使用模块方式 `python -m delete.orchestration_install` 运行。
...
- 与对应 collect 一致：未显式指定日期时，结束日期取对话中的月报日期，开始日期向前推 3 个日历月的同一日期。采集默认按 3 天拆分；清理用原请求总范围，并核对其包含的全部分段批次，不传 --chunk-days。
...
```powershell
py -3.11 -m delete.orchestration_install --database data/quality_assessment.db --start-date 2026-05-31 --end-date 2026-08-31
```
