...
description: 从 MySQL 源快照计算线上口径的终端回收率，生成完整 Excel、两张汇总 sheet 的 JSON 及数据库指标。用户提到终端回收率汇总、终端回收算数或完整终端回收流程时使用。
---
...
三个 collector 须使用 `database` 或 `both`，或将用户确认周期的本地文件以 `--reuse-existing` 入库。仅有 `data/raw/` 下的文件不等于数据库具备源数据。
...
```powershell
py -3.11 metrics/terminal_recovery/terminal_recovery_export_online.py --start-date 2026-08-01 --end-date 2026-08-31 --mode both --output-dir D:/MonthlyReport/outputs/terminal_recovery_online
```
