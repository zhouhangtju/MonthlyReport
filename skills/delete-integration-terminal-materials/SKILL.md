---
name: delete-integration-terminal-materials
description: 清理数据库中终端出入库、物资基准库及物料名称映射数据及关联明细，保留指标结果和本地文件。用户明确要求清理 integration_terminal_outbound、integration_terminal_inbound、integration_material_baseline、terminal_material_names 时使用；不在取数、算数或月报流程中自动调用。
---

# 一体化：终端回收物资数据清理

只负责终端出入库、物资基准库及物料名称映射数据清理，不取数、不计算指标、不绘制 PPT。对应取数技能为 `collect-integration-terminal-materials`，只调用已有清理入口，不另写 SQL 删除。

## 数据口径

- 数据集：`integration_terminal_outbound`、`integration_terminal_inbound`、`integration_material_baseline`、`terminal_material_names`
- 源记录主键：沿用入库记录标识，不重新生成主键或去重；按已有记录与批次关系定位，不按主键重新计算业务口径。
- 日期口径：ETL 统计月，不使用接口延伸到次月的实际查询截止日。
- 关联清理范围：原始记录、版本、终端源快照及快照行数据、对应算数明细，仅删除选中批次和脚本已验证的关联记录，不清空共享表。
- 入口：`delete/integration_terminal_materials.py`，使用模块方式 `python -m delete.integration_terminal_materials` 运行。
...
- 与对应 collect 一致：同月起止日期，月报用完整自然月。接口实际延伸至次月 3 日，但 ETL 仍标记统计月；清理参数沿用统计月，不自行延伸至次月 3 日。
...
```powershell
py -3.11 -m delete.integration_terminal_materials --database D:/MonthlyReport/data/quality_assessment.db --start-date 2026-08-01 --end-date 2026-08-31
```
