...
| database | 两个汇总 sheet 的结构化指标写入 SQLite，不保留本地结果文件 |
| both | 完整 Excel + JSON + SQLite 结构化指标 |
...
```powershell
$database = "D:/MonthlyReport/data/quality_assessment.db"
...
通过公共 `storage/importer.py` 导入 raw 记录，并保存 `tr_source_snapshot`、`tr_source_row`，以保留原行序、重复记录、类型和样式。
...
- `ads_metric_result`：仅保存两个指定 sheet 的数值和维度，metric_code 分为 `terminal_recovery_rate`、`terminal_recovery_count`。
