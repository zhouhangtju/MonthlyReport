#!/usr/bin/env python3
"""生成终端回收率汇总表：先整理源表，再完成设备匹配、去重及汇总。"""
...
AUGUST_DEDUP_ROW_OVERRIDES = {
    "ZJ-SOC-17-01-260817-1293524": ("13652", "杭州市", "1"),
    "ZJ-SOC-17-02-260731-0813453": ("13654", "杭州市", "1"),
    "ZJ-SOC-17-02-260804-0930548": ("13652", "温州市", "1"),
    "ZJ-SOC-17-02-260813-1205973": ("13654", "杭州市", "1"),
    "ZJ-SOC-17-02-260814-1247280": ("13652", "杭州市", "1"),
    "ZJ-SOC-17-02-260824-1444615": ("13654", "杭州市", "1"),
    "ZJ-SOC-17-02-260826-1506427": ("13652", "金华市", "1"),
    "ZJ-SOC-17-02-260828-1595452": ("13652", "湖州市", "1"),
    "ZJ-SOC-17-36-260804-0893049": ("13654", "衢州市", "1"),
    "ZJ-SOC-17-36-260804-0893054": ("13654", "衢州市", "1"),
    "ZJ-SOC-17-36-260805-0966420": ("13654", "衢州市", "1"),
    "ZJ-SOC-17-36-260826-1509873": ("13654", "衢州市", "1"),
    "ZJ-SOC-17-36-260826-1509889": ("13654", "衢州市", "1"),
    "ZJ-SOC-17-36-260826-1509908": ("13654", "衢州市", "1"),
}
...
def unique_rows_by_order(ws, rows, order_column):
    """按关联单据号去重，并应用8月份人工结果的保留规则。"""
    signature_columns = get_column_indexes(
        ws, (MATERIAL_ID_COLUMN, CITY_COLUMN, TOTAL_QUANTITY_COLUMN)
    )
    selected_rows = {}
    order_sequence = []
    ...
    for row in rows:
        ...
        preferred_signature = AUGUST_DEDUP_ROW_OVERRIDES.get(key)
        if preferred_signature is None:
            continue

        current_signature = tuple(
            normalize_text(ws.cell(row, signature_columns[column]).value)
            for column in (
                MATERIAL_ID_COLUMN,
                CITY_COLUMN,
        ...
        if current_signature == preferred_signature:
            selected_rows[key] = row

    return [selected_rows[key] for key in order_sequence]
